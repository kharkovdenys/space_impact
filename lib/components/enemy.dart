...
class Enemy extends SpriteComponent
    with HasHitboxes, Collidable, HasGameRef<SpaceImpact> {
  ...
  Enemy({Sprite? sprite, Vector2? position, Vector2? size, int? type})
    ...
    _timer=Timer(0.8,onTick: () {_timer.stop();});
  }
  ...
  void update(double dt) {
    ...
    if(_type==1&&!_timer.isRunning()&&(position.y-gameRef.player.y).abs()<=gameRef.canvasSize.y/5&&position.x>=gameRef.player.x){
      gameRef.add(EnemyProjectile(sprite: gameRef.spriteSheet.getSpriteById(6),position: position-Vector2(16, 0),size: Vector2(64, 64)));
      _timer.start();
    }
  }
